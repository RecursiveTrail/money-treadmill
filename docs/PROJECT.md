...
**Read this first** when changing gameplay, money math, the tick loop, or UI that reads game state. The full v1 design is in `docs/superpowers/specs/2026-09-18-middle-class-treadmill-design.md`. The live optional house/car/marriage/kid, net-worth win, and balance-sheet HUD rules are in `docs/superpowers/specs/2026-09-19-optional-life-commitments-design.md`; that spec wins where it changes v1. The original PRD is `docs/prd.md` (several loops there are outdated — spec + this file win).
...
1. `beginMonth` — `needsAnnualBoss = ageMonths === 0 && yearsPlayed > 0`; paycheck pays `livingExpenses + rent + EMIs` and amortizes loans; if `rng() < 0.30` pause on `awaitingEvent`.
...
- 15 life events in `src/engine/events.ts`. Uniform pick after the 30% roll.
- Annual bosses in `src/engine/bosses.ts`: years 1–3 named; year 4+ is `(yearsPlayed - 4) % 6`. `getAnnualBoss` throws if `yearsPlayed < 1`.
- House, car, marriage, and one child are optional. January/July flavour taunts cost ₹0 and never displace a real offer.
...
| `src/engine/economy.ts` | `liquidate`, paycheck/bills/SIP, `transferToMarket` |
| `src/engine/events.ts` / `bosses.ts` / `ending.ts` | Content + receipt math |
| `src/engine/loans.ts` | EMI calculation, loan origination/amortization, down-payment affordability |
...
| `src/ui/EventModal.tsx` / `EndReceipt.tsx` | Overlays |
| `src/ui/ChoiceModal.tsx` | House/car/marriage/kid offers and taunts |
...
- App phases: `setup` → form; `playing` / `awaiting*` → dashboard; event, boss, and choice phases → modal; `ended` → receipt only.
